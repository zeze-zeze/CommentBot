...
Output only your reply (no quotes, no prefixes or explanations).`,
    ...
    warn_autosubmit:
      "⚠️ This mode posts AI-generated replies automatically, with no chance to review each one. Automated posting may violate the platform's terms — only use it on content you own, at your own risk.",
    ...
    err_bad_url: '✗ Invalid API URL',
    err_perm_denied: '✗ Access to {host} was denied — the custom endpoint needs it to work',
    warn_http_cleartext:
      '⚠️ Plain http endpoint: your API key and comment text are sent unencrypted. Use https unless this is a local endpoint.',
    ...
    confirm_autosubmit:
      '"{name}" will automatically POST AI-generated replies on your behalf — you will not review each one, and posts cannot be undone.\n\nAutomated posting may breach the platform\'s terms of service. Only enable this on content you own, and use it responsibly.\n\nEnable it?',
    ...
    st_ctx_invalid: 'Extension was reloaded — please refresh this page and try again',
    // background 錯誤
    ...
    err_no_perm: 'No access to {host}. Open the popup and click "Test connection" to grant it.',
...
請直接輸出你要回覆的內容（不要加引號、不要加任何前綴說明）。`,
    ...
    warn_autosubmit:
      '⚠️ 此模式會自動張貼 AI 產生的回覆，且不會讓你逐則確認。自動張貼可能違反平台服務條款——請僅用於你自己擁有的內容，並自負風險。',
    lbl_persona: '補充指示（選填）',
    ...
    st_testing: '測試中…',
    st_test_ok: '✓ 連線成功',
    err_bad_url: '✗ API 網址無效',
    err_perm_denied: '✗ 已拒絕存取 {host}，自訂端點需要此權限才能運作',
    warn_http_cleartext:
      '⚠️ 未加密的 http 端點：你的 API 金鑰與留言內容會以明文傳送。除非是本機端點，建議改用 https。',
    ...
    confirm_autosubmit:
      '「{name}」會自動替你張貼 AI 產生的回覆——你不會逐則確認，且送出後無法復原。\n\n自動張貼可能違反平台的服務條款。請僅在你自己擁有的內容上啟用，並負責任地使用。\n\n確定要啟用嗎？',
    ...
    st_ctx_invalid: '擴充功能已重新載入，請重新整理此頁面後再試',
    err_no_key: '尚未設定 {label} 的 API Key（請點擴充功能圖示進行設定）',
    err_no_url: '尚未設定 API URL（請點擴充功能圖示進行設定）',
    err_no_perm: '尚未取得 {host} 的存取權，請開啟彈出視窗並按「測試連線」以授權。',
...
function normalizeSettings(raw) {
  ...
  const out = {
    ...
    platforms: {
      ...
      twitter: !(raw.platforms && raw.platforms.twitter === false),
      threads: !(raw.platforms && raw.platforms.threads === false),
    },
